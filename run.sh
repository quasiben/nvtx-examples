...
nsys profile -t nvtx,cuda --force-overwrite=true --stats=true --output=numba-jit python nvtx-numba-jit.py 
...
MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 nsys profile -t nvtx,osrt,cuda --stats=true --force-overwrite=true --output=cudf python nvtx-cudf.py

# requires patched cpython
# nsys profile -t nvtx,osrt,cuda --stats=true --force-overwrite=true --output=cpython-nvtx-patch python nvtx-dictionary.py